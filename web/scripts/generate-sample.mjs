...
for (const g of groups) {
  ...
  const status = role === 'unused' ? 'unused' : (onAce && !reachable ? 'dormant' : 'active');
...
const nDormant = groups.filter(g => g.status === 'dormant').length;
...
console.log(`[generate-sample] folders=${folders.length} groups=${groups.length} (access=${nAccessGroups} role=${nRoleGroups} dormant=${nDormant}) users=${users.length} aces=${aces.length}`);
