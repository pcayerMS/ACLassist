...
for (let i = 0; i < 10; i++) rbacAssignments.push({ principalId: randomUUID(), principalType: i % 2 ? 'User' : 'Group', roleDefinitionName: roleNames[i % roleNames.length], scope: '/subscriptions/xxxx/resourceGroups/rg-lab1-adlsacl-cac/providers/Microsoft.Storage/storageAccounts/stlab1adlsaclcac' });

const memberOf = new Set([...memberships.map(m => m.groupId), ...nesting.map(n => n.parentGroupId)]);
const referenced = new Set(aces.filter(a => a.isNamed && a.principalType === 'group').map(a => a.principalId));
const orphanGroups = groups.filter(g => !memberOf.has(g.id) || !referenced.has(g.id))
  .map(g => ({ id: g.id, displayName: g.displayName, emptyMembers: !memberOf.has(g.id), notOnAnyAce: !referenced.has(g.id) }));

const inventory = {
  meta: {
    tool: 'adls-acl-assessment', phase: 1, engineVersion: '0.1.0-sample', generatedUtc: new Date().toISOString(),
    target: { tenantId: 'demo.onmicrosoft.com', subscriptionId: '00000000-0000-0000-0000-000000000000', resourceGroupName: 'rg-lab1-adlsacl-cac', storageAccount: 'stlab1adlsaclcac', fileSystem: 'adls-aircanada', rootPath: '/' },
    auth: { mode: 'interactive', account: 'demo@contoso.com' },
    counts: { folders: folders.length, aces: aces.length, groups: groups.length, users: users.length, memberships: memberships.length, groupNesting: nesting.length, rbacAssignments: rbacAssignments.length, orphanGroups: orphanGroups.length, staleUsers: 0 },
...
console.log(`[generate-sample] folders=${folders.length} groups=${groups.length} users=${users.length} aces=${aces.length} orphans=${orphanGroups.length}`);
